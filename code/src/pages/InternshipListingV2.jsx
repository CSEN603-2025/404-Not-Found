...
function InternshipListingV2() {
  ...
  const filteredInternships = internships.filter((internship) => {
    ...
    const matchesIndustry = !industryFilter || internship.title === industryFilter;
    const matchesDuration = !durationFilter || internship.Duration === durationFilter;
    const matchesPaid = !paidChecked || internship.Type === 'Paid';
      ...
      <div className="main-table-container">
        ...
        <div className="filter-controls" style={{ display: 'flex', gap: '1rem', margin: '1rem 0' }}>
          <select value={industryFilter} onChange={e => setIndustryFilter(e.target.value)}>
            ...
            {[...new Set(internships.map(i => i.title).filter(Boolean))].map(title => (
              <option key={title} value={title}>{title}</option>
            ))}
          </select>
          <select value={durationFilter} onChange={e => setDurationFilter(e.target.value)}>
            <option value="">All Durations</option>
            {[...new Set(internships.map(i => i.Duration).filter(Boolean))].map(dur => (
...
export default InternshipListingV2;